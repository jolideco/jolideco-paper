...
% Abstract with filler text
    ...
    \subsection{Some notes on Deep Learning Methods}
    ...
    faster than the speed of light (structure in the CMB is an exception here). This means on smaller scales astronomical
    ...
    We compare the performance of \jolideco with different prior assumptions and against the \textit{LIRA} method introduced by \cite{Esch2004}. More specifically we use the following definitions:
    ...
    Figure \ref{fig:comparison-scenarios} shows the comparison of the methods for a selected subset of scenarios A1, B3, C3 and D3. 
    
    On scenario A1 all methods show acceptable to excellent results for the isolated point sources in the upper right of the image. This demonstrates all methods take into account the PSF and background model model correctly. The early stopped uniform prior still shows an extension for those cases, because the optimization process has not finished. The \jolideco with the Zoran-Weiss patch prior also results in less sharp images, because the prior has been learned on natural images of landscapes, objects, every day scenes etc. which do not include point sources. The \jolideco with the prior learned from GLEAM data, which also includes point sources, shows much improved results.

    In the lower left of the image the point sources become fainter and closer, challenging all methods in succeeding to separate those as independent sourcea. The uniform prior optimized "to end" shows excellent results, still separating the sources. 
    
    For scenario B3 all method recover the point sources arpund the central extend source. 
    ...
    Scenario C3 challenges the methods to reconstruct a weak extended source under four brighter point sources. While again all methods reconstruct the point sources well, the visual differences are large for the weal underlying extended emission.
    ...
    \todo{Should we leave the background norm free?}
    Figure \ref{fig:comparison-signal-noise} shows the comparison for the method "Jolideco GLEAM v0.1" for varying signal to noise ratio for the "D" scenario.

    Some observations:
    
    \begin{itemize}
        \item as expected the reconstruction quality improves with stronger signal
