...
% Author list
    ...
    \cite{Selig2015A} and \cite{Pumpe2018} approach the problem of deconvolution of Poisson noise data implicitly using the framework of information field theory (IFT). Based on physical prior assumptions on the spatial distribution of point sources and local correlation structures of diffuse emission, they are able to deconvolve \fermi observations into a mixture of point-like and diffuse emission components. While their method works well for large-scale diffuse emission, such as the Galactic diffuse gamma-ray emission, it shows artifacts around the Galactic Plane, when applied to gamma-ray data from the Fermi-LAT \citep{Selig2015B}. 

    \cite{Westerkamp2024} applied the same IFT framework to a Chandra x-ray observation of the supernova remnant (SNR) SN1006. The authors demonstrated that it is possible to separate the point like emission of the extra-galactic background sources from the diffuse foreground emission of the SNR. The method delivered good reconstruction quality on synthetic data. When applied to observational data the authors found plausible reconstructions that include small artifacts, which they attributed to a mismatch between the modelled and actual PSF.
    ...
    \begin{equation}
        \label{eq:cash}
        \mathcal{C}\left( \mathbf{D}_j | \boldsymbol{\lambda}_j(\mathbf{x}) \right) = \sum_{i=1}^{N_j} \left(\lambda_{j,i} - D_{j, i} \log{\lambda_{j,i}}\right),
    \end{equation}
    %  
    By summing over all observations we obtain the (negative) joint log-likelihood of measuring a set of counts images $\mathbf{D}$ given an unknown flux image $\mathbf{x}$:
    %
    \begin{equation}
        \label{eq:joint}
        \mathcal{L}\left( \mathbf{D} | \mathbf{x} \right) = \sum_{j=1}^J \mathcal{C}\left( \mathbf{D}_j | \boldsymbol{\lambda}_j(\mathbf{x}) \right)
    \end{equation}
    %
    ...
    When reconstructing images in the context of inverse problems we are operating in a high dimensional parameter space, because each pixel in the image represents a parameter in the optimization process. High resolution \xray-images are typically composed of  
    %one can expect 
    up to $\mathcal{O}(10^6)$ pixels. By using prior information we can introduce correlations among the parameters, to reduce the \enquote{effective dimensionality} of the problem and guide the optimization process toward unique and more stable solutions.
    ...
    Taking the logarithm of Equation~\ref{eq:bayes}, replacing the definition of the likelihood $P(\mathbf{D}|\mathbf{x})$ with Equation~\ref{eq:joint} and dropping the normalization term $P(\mathbf{D})$ which is independent of $\mathbf{x}$, we obtain the following expression for the (negative) log-posterior distribution
    $\mathcal{L}$:
    %
    \begin{equation}
        \label{eq:total}
        \mathcal{L}\left(\mathbf{x} | \mathbf{D} \right) = \sum_{j=1}^J \mathcal{C}\left( \mathbf{D}_j | \boldsymbol{\lambda}_j(\mathbf{x}) \right) - \beta \cdot \mathcal{P}(\mathbf{x}),
    \end{equation}
    %
    where $\mathcal{C}\left( \mathbf{D}_j | \boldsymbol{\lambda}_j(\mathbf{x}) \right)$ represents the summed log-likelihood for an individual observation $j$ and $\mathcal{P}(\mathbf{x})$ represents the log of the prior distribution with the factor $\beta$ adjusting the weight of the prior relative to the joint log likelihood term. The parameter $\beta$ can be viewed as a hyper-parameter, chosen by the user to adjust the strength of the prior distribution relative to the likelihood. The choice of $\beta$ is optional and for all experiments presented in this paper we set $\beta=1$. 
    ...
    A patch is defined as a small, typically square, region of size $N_p \times N_p$ pixels, representing a local neighborhood in a given image. Mathematically we can formulate the extraction of such patches as:
    %
    \begin{equation}
        \mathbf{x}_n = \mathbf{P}_n \mathbf{x},
    \end{equation}
    %
    where $\mathbf{P}_n$ represents a neighborhood matrix to extract the $n$-th patch from the image $\mathbf{x}$ and where $\mathbf{x}_n$ is the vector of length $N_p^2$ representing the \enquote{flattened} image patch. By obtaining a large number of patches we can model the distribution of pixel intensities within the patches using a Gaussian Mixture Model (GMM) \citep{McLachlan2004}. The GMM is a universal function approximator and thus can model arbitrary distributions. For now we assume that we have a pre-trained GMM which encodes the patch distribution of some reference image we have chosen. During the actual image reconstruction we aim to identify the most likely GMM component per patch, treating this as a discrete nuisance-parameter that is optimized along with the image $\mathbf{x}$. 
    ...
    \end{equation}
    where $\mathcal{N}$ represents a multivariate Gaussian normal distribution, $\mathbf{\mu_k}$ is the vector of Gaussian means (with dimension equal to the number of pixels in each patch), $\mathbf{\Sigma}_k$ is the variance-covariance matrix of the Gaussian distribution (which tabulates the variances and pairwise covariances of the patch pixels), and  $\pi_k$ is the weight of component $k$ of the GMM.
    ...
    \end{equation}   
    %
    where $\mathbf{P}_n$ is again the matrix that extracts the $n$-th patch from the image $\mathbf{x}$ and $\pi_{\hat{k}_n}$ is the weight of GMM component $\hat{k}_n$, with $\hat{k}_n$ being the index of the GMM component that maximizes the log-prior distribution for patch $n$.
 ...
 \begin{figure}
    ...
    \begin{equation}
        \label{eq:patch-norm}
        \widetilde{\mathbf{P}_n\mathbf{x}}
        = \frac{\mathbf{x}_n - \Bar{\mathbf{x}}_n}{\Bar{\mathbf{x}}_n}.
    \end{equation}
    %
    ...
    \begin{equation}
        ...
        \begin{aligned}
        \boldsymbol{\lambda}_j(\mathbf{x}, \delta_{x,j}, \delta_{y,j}, \alpha_j) = \mathbf{E}_j \cdot  \left(\mathrm{PSF}_j \circledast \phi_j(\mathbf{x}| \delta_{x,j}, \delta_{y,j})\right) \\ + \alpha_j \cdot \mathbf{B}_j
        \end{aligned}
    \end{equation}
    %
    ...
    Modifying the (negative) log-posterior distribution given in Equation~\ref{eq:total} for the normalization of the $\mathbf{x}_n$ in Equation~\ref{eq:patch-norm}  and the adjustment for systematic effects in Equation~\ref{eq:model-npred-calibration} yields,
    \begin{equation}
        \label{eq:total-final}
        \begin{aligned}
        \mathcal{L}\left(\mathbf{x}, \mathbf{k}, \boldsymbol{\delta}, \boldsymbol{\alpha} | \mathbf{D} \right) 
        = \sum_{j=1}^J \mathcal{C}\left( \mathbf{D}_j | \boldsymbol{\lambda}_j(\mathbf{x}, \delta_{x,j}, \delta_{y,j},\alpha_j) \right) 
        \\ - \beta \left\{
         \sum_{n = 1}^N \mathcal{P}_{\rm GMM}(\widetilde{\mathbf{P}_n \mathbf{x}} | {k}_n) +
        \sum_{n = 1}^N \log \pi_{{k}_n} \right\},
        \end{aligned}
    \end{equation}
    where $\mathbf{k} = (k_1, \ldots k_N)$, $\boldsymbol{\delta} = (\delta_{x,1},\delta_{y,1},\ldots, \delta_{x,J},\delta_{y,J})$ and $\boldsymbol{\alpha} = (\alpha_1,\ldots, \alpha_J)$. For fixed $\mathbf{x}$, Equation~\ref{eq:total-final} is optimized as a function of $\mathbf{k}$ by Equation~\ref{eq:khat}, but with $\mathbf{x}_n$ replaced by $\widetilde{\mathbf{P}_n \mathbf{x}}$.  The joint prior distribution of $(\mathbf{x}, \mathbf{k}, \boldsymbol{\delta}, \boldsymbol{\alpha})$ is given by the term in Equation~\ref{eq:total-final} within curly brackets. Note that this places uniform prior distributions on $\boldsymbol{\delta}$ and $\boldsymbol{\alpha}$ and formally sets the prior on $\mathbf{P}_n\mathbf{x}$ to the GMM evaluated at  $\widetilde{\mathbf{P}_n \mathbf{x}}$. Since this does not depend on $\mathbf{\bar x}_n$, we implicitly assume a uniform prior on each $\mathbf{\bar x}_n$. In practice, we do not expect the choice of prior distribution on the scale of each patch to influence the final fit, but some users may prefer to assume a flat prior on the log of each $\mathbf{\bar x}_n$ by adding $\sum_{n=1}^N \mathbf{\bar{x}}_n$ within the curly brackets on the right-hand-side of Equation~\ref{eq:total-final}. 
    ...
    How well these bootstrap error estimates approximate the true posterior uncertainty and/or the frequency coverage of the derived bootstrap intervals is a topic of future study. 
    ...
    \subsection{Learning the GMMs}
    \label{ssec:jolideco-gmm-library}
    The patch-based image prior described in Section~\ref{sec:patch-prior} requires a pre-trained GMM. We train the GMM by choosing a set of reference images, splitting them into patches of size $8\times8$ pixels and learning a GMM with $K=128$ components from the whole distribution of patches. In general the strength of the GMM prior is its high \enquote{expressiveness}. This means the prior distribution contains a large variety of structural shapes and has freedom to adapt to the data in the optimization process by choosing the most likely GMM component for each patch in the image. \cite{Zoran2012} found empirically that the reconstruction quality depends on the number of GMM components. A higher number of components clearly leads to improved reconstruction quality, as the GMM was more expressive. This also aligns with the intuitive argument that GMMs are universal function approximators; with a sufficiently large number of components they can approximate any distribution. \cite{Zoran2012} found the reconstruction quality begins to saturate with about 32 components, at least for natural images. We decided to adopt a more conservative number with 128 components.
    
    With radio data, \cite{Bouman2016} found their results to be mostly independent of the choice of data used for learning the GMM prior. In the low signal-to-noise regime, however, we typically expect a higher dependence of the results on the choice of prior, simply because the prior distribution is more influential when the data is less informative. Thus, there is the potential for the choice of GMM training data to be influential on results and the prior should be adapted to the particular analysis scenario in order to avoid bias. For this reason and for purposes of testing we learned and provide a selection of GMMs to be used with the patch prior for download in a \textit{GitHub} repository\footnote{\url{https://github.com/jolideco/jolideco-gmm-prior-library}}; illustrative examples of the training images are also provided in this repository.
    ...
    Maximising the \aposteriori likelihood defined by Equation~\ref{eq:total-final} with respect to the model parameters $\mathbf{x}$ is a high dimensional optimization problem. We found the convergence greatly improved when the optimizer operates on $\log{\mathbf{x}}$. This is to be expected, as the brightness in pixels of astronomical image can span multiple orders of magnitudes. We used the ADAM algorithm \citep{Kingma2014}, a stochastic gradient descent method, without decay and a fixed learning rate of $\mathrm{lr}=10^{-3}$. In practice we found the convergence of the method to be very stable, however we applied the following strategy to speed up the optimization process:

    \begin{itemize}
        \item[1.] We start from a random image where each pixel was drawn from a normal distribution with the mean corresponding to the total mean of the flux in the image and a standard deviation of 10\% of the mean. We first estimate $\mathbf{x}$ using a uniform prior and early stopping after 50 iterations. This corresponds approximately to a reconstruction obtained with the standard RL algorithm. As the uniform prior is much faster to evaluate ($\mathcal{O}(100)$ times faster), this step quickly produces a much better starting value for the optimization with the patch prior described in Step~3. 
        \item[2.] We freeze the image parameters and optimize the parameters associated with the systematic error described in Equation~\ref{eq:model-npred-calibration}. Again, this procedure quickly delivers better starting values for the optimization in Step 3. Computation is fast because the number of parameters is very small (and scales linearly with the number of datasets).
        \item[3.] Finally we simultaneously optimize the image $\mathbf{x}$, the GMM component indicators for each patch, $k_n$ and the parameters associated with systematic error until convergence.  Algorithmically, this is accomplished by iteratively updating (a) $\mathbf{k}$ for fixed $\mathbf{x}, \boldsymbol{\alpha}$, and $\boldsymbol{\delta}$ (via Equation~\ref{eq:khat} with $\mathbf{x}_n$ replaced by $\widetilde{\mathbf{P}_n\mathbf{x}}$) and (b) $(\mathbf{x}, \boldsymbol{\alpha}, \boldsymbol{\delta})$ for fixed $\mathbf{k}$ via SGD.
        Convergence is verified visually by inspecting the series of values of the log posterior distribution evaluated at the iterates or the models parameters. We adjust the number of epochs (number of passes through the whole set of observations) such that the curve had flattened out for the last few hundred iterations. This is also the point in the iteration, where the noise in the loss, introduced by SGD and the cycle spinning, clearly dominates any remaining decrease in the mean loss.
    \end{itemize}
    ...
    \centering
        ...
        \caption{Image metrics for the different source scenarios and methods shown in Figure~\ref{fig:comparison-scenarios}. The first number in each cell shows the structural similarity index (SSI, the closer to unity, the better) and the second number shows the normalized root mean square (NRMSE, the lower the better).}
        \label{tab:metrics}
    \end{table*}
    %
    ...
    \begin{table*}
    \centering
        \begin{tabular}{cc@{\hspace{2pc}}c@{\hspace{2pc}}c} 
            \input{output/metrics-table-bkg.tex}
        \end{tabular}
        \script{metrics-table-bkg.py}
        \caption{Image metrics for the different signal to noise ratios shown in Figure~\ref{fig:comparison-signal-noise}. The first number in each cell shows the structural similarity index (SSI, the closer to unity, the better) and the second number shows the normalized root mean square (NRMSE, the lower the better).}
        \label{tab:metrics-bkg}
    \end{table*}
    ...
    \textbf{Zoom B} shows the region around the putative central compact object of the SNR (\cite{Vogt2018}, \cite{Hebbar2020}, but \cite{Long2020}). While the emission is barely visible in the counts image it is clearly evident above the background in the \jolideco reconstruction. Visually the reconstruction does not result in a point source, but leaves a visible extension of a few pixels.
    
    However, at this level of analysis and signal to noise, both scenarios remain plausible: a real intrinsic extension of the feature, or a real point source with a residual extension in its reconstruction, owing to the low signal to noise. We observed the second possibility in
    the simulated point source dataset discussed in Section~\ref{subsec:test-datasets}\footnote{We note that, as illustrated in Section~\ref{subsec:methods} for Scenario~A, the remaining extension of the point source after deconvolution may be smaller if the source is brighter.}.
    ...
    To further check the quality of the reconstructed flux we compute a map of smoothed standardized residuals defined by
    \begin{equation}
    ...
    \end{equation}
    where $\mathbf{D}$ is the measured counts image, $\boldsymbol{\lambda}$ is the expected counts image,  and $g_{\sigma}$ is a Gaussian smoothing kernel (normalized to integrate to unity) with width $\sigma=5$ pixels. We apply the smoothing to enhance possible structures in the sparse data, while the normalisation factor ensures the smoothed residual pixel values are still distributed following a unit Gaussian distribution.
    
    The result is shown in the right panel of Figure~\ref{fig:example-fermi-lat}. The image shows no systematic effects and a spatially homogeneous distribution of the residual noise. The resulting distribution of residual significances per pixel is centered at $\mu=-0.001$ with a width of $\sigma=0.991$, indicating a close agreement with a standard normal distribution, as expected for a good model of the emission. In Figure~\ref{fig:example-fermi-lat-non-calibrated} we also show the residuals per PSF class with and without the correction defined by Equation~\ref{eq:model-npred-calibration}. More details of the \fermi analysis including results and residuals are available at \url{https://github.com/jolideco/jolideco-fermi-examples}.
    ...
    We imagine that \jolideco is the first step of a larger project building ML-inspired methods for the reconstruction of astronomical images that are dominated by Poisson noise. While \jolideco has already demonstrated excellent reconstruction quality in multiple scenarios, it still has significant potential for future extension and improvements.
    ...
    Working in the context of computer tomographic image reconstruction, \cite{Altekrueger2022} used normalising flows (NF) in their formulation of the patch prior in order to further improve the general expressiveness of the prior. As with a GMM, we can evaluate the NF-based prior distribution and thus construct an explicit posterior distribution for optimization and computation of the MAP estimate. \citet{Ulyanov2017} introduced another promising prior distribution, known as the \enquote{Deep Image Prior} and showed that prior structure for an image can be built into a deep neural network as an \enquote{inductive bias}, where the architecture of the network encodes the prior information on the correlation structure among neighboring pixels. The authors showed, that the deep image prior successfully imposes self similarity on structures in the image. As structures in astronomical images often show sell-similarity this is an interesting approach, that in principle could be combined with the patch prior.

    A second category of improvement to \jolideco is generalizing it to jointly model the energy and spatial domains, thus enabling hyperspectral image reconstruction. Instruments such as \fermi combine instrument responses that vary strongly with energy along with a limited overall energy resolution. This configuration introduces correlations between images in neighboring energy bands. Accounting for energy data in the reconstruction would allow us to handle these correlations and also open up the possibility of merging multiple \xray datasets in the joint spectral/spatial domain,  thus improving both  angular and energy resolution. 

    \jolideco can already handle an independent background component. However we plan to extend \jolideco to enable it to handle multiple source components at the same time. This allows us in principle, as shown by \cite{Selig2015A} and \citet{Pumpe2018} to separate extended sources from point-like sources, based on different prior assumptions on the spatial correlation and distribution of pixels for each model component. 

    A last category of improvement to \jolideco is computing principled error estimates for the reconstruction. In this paper, we use bootstrap methods. Since we can compute the full posterior distribution, however, we could build an approximation to the covariance matrix of the MAP estimate, for example, by using a low-rank or sparse representation. Alternatively we can use MCMC to obtain a sample from the full posterior distribution, enabling us to explore the distribution and quantify  uncertainty both in the parameters and functions of the parameters constructed to represent structures in the image. Estimating errors for the reconstruction would also include propagating systematic uncertainties on the PSF to the reconstructed image and handling a PSF that varies across the field of view, as recently proposed by \cite{Sakai2023}.    
    ...
    This work was conducted under the auspices of the CHASC International Astrostatistics Center. CHASC is supported by NSF grants DMS-21-13615, DMS-21-13397, and DMS-21-13605; by the UK Engineering and Physical Sciences Research Council [EP/W015080/1]; and by NASA APRA Grant 80-NSSC21-K0285.
    
    We thank CHASC members for many helpful discussions, especially Xiao-Li Meng and Katy McKeough.
    DvD was also supported in part by a Marie-Skodowska-Curie RISE Grant (H2020-MSCA-RISE-2019-873089) provided by the European Commission.
    ...
    This research has made use of data obtained from the Chandra Data Archive, contained in~\dataset[DOI: 10.25574/cdc.244]{https://doi.org/10.25574/cdc.244}. This research also made use of software provided by the Chandra X-ray Center (CXC) in the application packages CIAO \citep{Fruscione2006} and {\it Sherpa} \citep{Freeman2001,Burke2022}.
    ...
    \end{table}
     ...
     \begin{figure*}
        \begin{centering}
            ...
            \caption{
                Illustration of the residuals of the \fermi analysis example of \textit{RX J0852.0-4622} from Section~\ref{sec:fermi-lat-example}. All images show the standardized residuals as computed by Equation~\ref{eq:approx-sigma} and smoothed with a \textit{Gaussian} kernel of width 5~pixels $\approx 0.1$~degrees, similar to the size of the PSF. The upper row shows the result without calibration, the lower row shows the result with the calibration applied, according to Equation~\ref{eq:model-npred-calibration}. The columns represent the four different PSF event classes of \fermi. The color scale is shown in the right panel. More information on this analysis example can be found on \url{https://github.com/jolideco/jolideco-fermi-examples}. 
            }
