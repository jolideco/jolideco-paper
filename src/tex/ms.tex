...
\begin{figure*}
 ...
 \begin{figure}
    ...
    $\lambda_{Bkg}= 0.001, 0.01$ and \qty[mode = text]{0.1}{counts/pixel}.
    
    As expected the reconstruction quality improves with signal strength. This includes both the variation of the background as well as signal strength. \todo{sensitivity scales as approx signal / sqrt(Bkg)? so what what is the implication? } Visually the method converges to the ground truth for very high signal-to-noise ratio. Once the signal strength becomes very large there is no dependency of the result on the background level anymore. This especially valid for scenario D5
    ...
    \begin{figure*}
        ...
        \begin{centering}
            ...
            \caption{
               Flux and counts profile of the Zoom A example region. The associated $2\sigma$ error band estimated using the bootstrap method is shown transparent in the background.
            }
            \label{fig:chandra-e0102-zoom-a}
        \end{centering}
    \end{figure*}
    To get a more qualitative measurement of the improvement we compute a flux profile on the image reconstructed by \jolideco. For this we chose a region perpendicular to the filament shown in Zoom A of Figure~\ref{fig:example-chandra}. To estimate the errors we use the bootstrapping method. For each individual observation we sample from the event list with replacement. For each realization of the set of observation we run \jolideco and compute a flux profile on the reconstructed image. This way we obtain a distribution of profiles, which we can use to compute an error band. The result is shown in Figure~\ref{fig:chandra-e0102-zoom-a}.

    We find that the error is 

    We approximately expect the following relation between the width of the counts profile, the PSF and the flux profile:
