...
\as{There is no mention of stochastic gradient decent in the description of the method above, so this is the first time you talk about it... maybe it needs to be included when you describe solving the GMM model?}
    ...
    \vlk{We compute the full-width at half-max (FWHM) of each of the profiles, for both the original counts and the deconvolved intensities, and find that the deconvolved profile is narrower.  For the counts profile, FWHM$_C=0.63 \pm 0.09$~arcsec, where the error bar is obtained by computing the standard deviations over the bootstrap sample; and the deconvolved intensity profile has FWHM$_J=0.56 \pm 0.03$~arcsec.  Note that the PSF itself has a FWHM$_{{\rm PSF}\approx{0.6}}$~arcsec\footnote{See projected radial profiles of the on-axis PSF at \url{https://space.mit.edu/cxc/marx/tests/PSF.html}.  The 1$\sigma$ radius is $\approx$0.26~arcsec (corresponding to 39\% enclosed counts in 2-D), suggesting that FWHM$_{{\rm PSF}}\approx{2.355{\cdot}0.26=0.62}$~arcsec.}.  }
    The profile computed from the images reconstructed with \jolideco has a much smaller noise level and the peak is much more pronounced. By fitting the profile with a univariate spline and finding its roots we measure the full width half maximum of the curves. We find $w_J=0.56\pm0.03$ for the \jolideco profile and $w_C=0.63\pm0.09$ for the profile computed on the counts image. Assuming that the PSF and the shape of peak of the profile are approximately Gaussian a we expect the following relation between the width of the counts profile, the PSF and the flux profile:

    \begin{equation}
        w_{J} = \sqrt{w_C^2 - \w_{\mathrm{PSF}}^2}
    \end{equation}

    With $w_{\mathrm{PSF}} \approx 0.25 \mathrm{arcsec}$ \footnote{The 38\% containment radius of the Chandra PSF is $\sigma_{39}=2$ native pixels. This corresponds to  ee e.g. \url{https://space.mit.edu/cxc/marx/tests/PSF.html}}
