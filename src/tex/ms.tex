...
% Abstract with filler text
    ...
    the observation specific exposure $\mathbf{E}_j$ and background emission $\mathbf{B_j}$ can be
    taken into account.
    ...
    $\mathbf{D_j}$ follows a Poisson distribution, the likelihood $\ell_j$
    of obtaining the measured image from a model image of the expected
    counts $\boldsymbol{\lambda_j}$ with $N$ pixels is given by:

    \begin{equation}
        \label{eq:poisson}
        \ell\left( \mathbf{D_j} | \boldsymbol{\lambda_j} \right) = \prod_{i=1}^N \frac{{e^{ - D_{j,i} } \lambda_i ^ {D_{j,i}}}}{{D_{j,i}!}}
    \end{equation}
    ...
    By summing over all observations we finally get the joint log-likelihood of measuring a set of counts images $\mathbf{D}$ given a unknown flux image $\mathbf{x}$:
    ...
    Using the Maximum Likelihood Estimation (MLE) we could estimate 
    ...
    \begin{equation}
        \label{eq:bayes}
        P(\mathbf{x}|\textbf{D}) = \mathcal{P}(\mathbf{x} ) \frac{\mathcal{L}(\textbf{D} |\mathbf{x})}{ \mathcal{P} (\textbf{D})}
    \end{equation}

    The total objective function $\mathcal{L}$ is given by:
    \begin{equation}
        \label{eq:total}
        \mathcal{L}\left( \mathbf{d} | \mathbf{x} \right) = \sum_m^M \mathcal{C}\left( \mathbf{d_m} | \mathbf{x} \right) - \beta \cdot \mathcal{P}(\mathbf{x})
    \end{equation}
    ...
    natural images. This is mostly due to the existence of real point sources, which rarely occur
    in natural images, but a very common in astronomical images because of compact objects 
    at high distances. The high dynamic range challenges any image reconstruction
    algorithm. 
    ...
    in the same dynamic range between 0 and 1. 
    ...
    This includes e.g. the absolute position of the sources in the image and the
    normalisation of the background image, which can vary from observation to observation.
    ...
    which is specific for each observation. This gives the following modified model definition:
    ...
    \begin{itemize}
        ...
        \item First optimize the systematic error parameters
        \item The go for the optimization uisng the patch prior, which can be computationally costly
    \end{itemize}
