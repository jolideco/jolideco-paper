...
% Author list
    ...
    \cite{Selig2015A} and \cite{Pumpe2018} approach the problem of deconvolution of Poisson noise data implicitly using the framework of information field theory. Based on physical prior assumptions on the spatial distribution of point sources and local correlation structures of diffuse emission, they are able to deconvolve \fermi observations
    into a mixture of point-like and diffuse emission components. While their method works well for large-scale diffuse emission, such as the Galactic diffuse gamma-ray emission, it showed artifacts around the Galactic Plane, when applied to gamma-ray data from the Fermi-LAT \cite{Selig2015B}.
    ...
    Where $\mathcal{N}$ is a multivariate Gaussian normal distribution. The distribution is parameterized by the vector $\mathbf{\mu_k}$, which a dimension equivalent to the number of pixels in a patch and represents the mean of each Gaussian component. The pairwise correlation between pixels is represented by the corresponding covariance matrix $\mathbf{\Sigma}_k$, while the weight of each component is given by $\pi_k$.
    ...
    \end{equation}   
    %
    where $\mathbf{P}_n$ is again the matrix that extracts the $n$-th patch from the image $\mathbf{x}$ to be reconstructed and $\pi_{\hat{k}_n}$ the weight of the component $\hat{k}_n$, which maximizes the log-likelihood for the n-th patch.
 ...
 \begin{figure}
    ...
    \begin{equation}
        ...
        \boldsymbol{\lambda}_j = \mathbf{E}_j \cdot  \left(\mathrm{PSF}_j \circledast \phi_j(\mathbf{x}| \delta_{x,j}, \delta_{y,j})\right) + \alpha_j \cdot \mathbf{B}_j
    \end{equation}
    %
    ...
    The patch-based image prior described in Section~\ref{sec:patch-prior} requires a pre-trained GMM. We train the GMM by choosing a set of reference images, splitting them into patches of size $8\times8$ pixels and learning a GMM with $K=128$ components from the whole distribution of patches. In general the strength of the GMM prior is its high \enquote{expressiveness}. This means the prior distribution contains a large variety of structural shapes and has freedom to adapt to the data in the optimization process by choosing the most likely GMM component for each patch in the image. \cite{Zoran2012} empirically found that the reconstruction quality depended on the number of components of the GMM. A higher number of components clearly lead to an improved reconstruction quality, as the GMM was more expressive. This also aligns with the intuitive argument that GMMs are universal function approximators; with a sufficient number of components they can approximate any distribution. \cite{Zoran2012} found the reconstruction quality started to saturate once the number of components was large enough. Empirically they measured a number of 32 components to be sufficient in practice for natural images. We decided to adopt a more conservative number of 128.
    
    With radio data, \cite{Bouman2016} found their results to be mostly independent of the choice of data used for learning the GMM prior. However, in the low signal-to-noise regime we typically expect a higher dependence of the results on the choice of prior, simply because the prior distribution is more influential when the data is less informative. Thus, there is the potential for the choice of GMM training data to be influential on results and the prior should be adapted to the particular analysis scenario in order to avoid bias. For this reason and for purposes of testing we learned and provide a selection of GMMs to be used with the patch prior for download in a \textit{GitHub} repository\footnote{\url{https://github.com/jolideco/jolideco-gmm-library}}. 
    ...
    \begin{itemize}
        \item[1.] We start from a random image where each pixel was drawn from a normal distribution with the mean corresponding to the total mean of the flux in the image and a standard deviation of 10\% of the mean. We first estimate $\mathbf{x}$ by using a uniform prior and early stopping after 50 iterations. This corresponds approximately to a reconstruction obtained with the standard RL algorithm. As the uniform prior is much faster to evaluate ($\mathcal{O(100)}$), we quickly arrive at a much better starting estimate for the reconstructed image. 
        \item[2.] We freeze the image parameters and optimize the parameters associated with the systematic error described in Equation~\ref{eq:model-npred-calibration}. This way we arrive at a better starting estimate for those. Again the process is fast, because the number of parameters is very small. 
    ...
    \centering
        ...
        \caption{Image metrics for the different signal to noise ratios shown in Figure~\ref{fig:comparison-signal-noise}. The first number in a cell shows the structural similarity index (SSI, the closer to unity, the better) and the second number shows the normalized root mean square (NRMSE, the lower the better).}
    \end{table*}
    ...
    \begin{equation}
        ...
        \rho(\mathbf{D}, \boldsymbol{\lambda}) \approx \frac{1}{\sqrt{\pi \sigma^2}}\frac{(\mathbf{D} - \boldsymbol{\lambda}) \circledast g_{\sigma}} {\sqrt{\boldsymbol{\lambda}\circledast g_{\sigma}}}.
    \end{equation}
    
    where $\mathbf{D}$ is the measured counts image, $\boldsymbol{\lambda}$ the predicted counts image  and $g_{\sigma}$ is a Gaussian smoothing kernel of width $\sigma=5$ pixels. The Gaussian smoothing kernel approximates the shape and size of the \fermi PSF. Together with the definition above, the standardized residuals approximately represent the significance of detecting a point source at the given pixel. Structures smaller than the PSF cannot be real.
    ...
    Estimating errors for the reconstruction would also include propagating systematic uncertainties on the PSF to the reconstructed image and handling a PSF that varies across the field of view, as recently proposed by \cite{Sakai2023}.    
